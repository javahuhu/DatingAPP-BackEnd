 ...
 */
function getFrontendOrigin(frontendUrl) {
    try {
...
async (_accessToken, _refreshToken, profile, done) => {
    try {
        ...
        if (!user)
            return done(new Error("Failed to create/find user"), undefined);
 ...
 */
router.get("/google/callback", passport.authenticate("google", { session: false, failureRedirect: "/api/auth/failure" }), async (req, res, next) => {
    try {
...
function renderPopupResponse(token, frontendUrl) {
