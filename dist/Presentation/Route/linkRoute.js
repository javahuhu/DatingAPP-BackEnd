...
const MAGIC_EXPIRES = (process.env.MAGIC_EXPIRES_IN ?? '15m');
const LOGIN_EXPIRES = (process.env.JWT_EXPIRES_IN ?? '1d');
...
const transporter = smtpHost && !Number.isNaN(smtpPort)
...
router.post('/magic-request', async (req, res) => {
    try {
        ...
        const payload = { email: normalized, type: 'magic' };
        const token = jwt.sign(payload, JWT_SECRET, { expiresIn: MAGIC_EXPIRES });
...
router.get('/magic', async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            user = await repo.save(persistence);
        }
        if (!user || !user.id) {
            console.error('Failed to ensure user exists for magic link', user);
            return res.status(500).send('Internal server error');
        }
        const loginPayload = { id: user.id, email: user.email };
        const loginToken = jwt.sign(loginPayload, JWT_SECRET, { expiresIn: LOGIN_EXPIRES });
